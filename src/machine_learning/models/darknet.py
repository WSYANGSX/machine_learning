...
class FPN(BaseNet):
    def __init__(self, num_anchors: int, num_classes: int) -> None:
        ...
        self.channels = (5 + self.num_classes) * self.num_classes
    ...
    def forward(self, skips: list[torch.Tensor]) -> torch.Tensor:
        ...
        # ----- 第一层检测 -----
        x3 = self.fpn.conv_x3(skips.pop())  # (1024,13,13) -> (512,13,13)
        detection3 = self.fpn.head_conv3(x3)  # (512,13,13) -> (255,13,13)
        x3_up = self.fpn.upsample(x3)  # (512,13,13) -> (512,26,26)

        # ----- 第二层检测 -----
        x2 = torch.cat([x3_up, self.fpn.conv_x2(skips.pop())], dim=1)  # (512,26,26) cat (256,26,26) -> (768,26,26)
        detection2 = self.fpn.head_conv2(x2)  # (768,26,26) -> (255,26,26)
        x2_up = self.fpn.upsample(x2)  # (768,26,26) -> (768,52,52)

        # ----- 第三层检测 -----
        x1 = torch.cat([x2_up, self.fpn.conv_x1(skips.pop())], dim=1)  # (768,52,52) cat (128,52,52) -> (896,52,52)
    ...
    def view_structure(self):
        ...
        # 定义适配函数（正确写法）
        def forward_wrapper(model, x):
            return model(x)  # 直接返回模型输出
